#' Plot true positives versus false positive
#'
...
#' plotTruePositivesVsFalsePositives(calcAccurateAmongTopK(res.proc, k = 5))
#' plotTruePositivesVsFalsePositives(calcAccurateAmongTopK(res.proc, k = 1), filename = "test.pdf")
plotTruePositivesVsFalsePositives <- function(occupationalPredictionsAmongTopK, filename = NULL) {

  print("Make sure this is the size of your test data. (or count the missing one as additional false positives on the upper right)")
  print(occupationalPredictionsAmongTopK[, .N, by = method.name])

  # calculate coordinates to plot
  n.true.positives.vs.n.false.positives <- occupationalPredictionsAmongTopK[, .SD[order(pred.prob, decreasing = TRUE), list(y = cumsum(acc), x = cumsum(1 - acc))], by = method.name]

  p <- ggplot(n.true.positives.vs.n.false.positives, aes(x = x, y = y, colour = method.name, group = paste(method.name))) +
    ...
    labs(x = "No. of False Positives", y = "No. of True Positives") +
