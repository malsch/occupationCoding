...
#' }
#' @param coding_index_without_codes a preprocessed character vector, similar to \code{coding_index_w_codes$title}.
...
#' }
#' @param dist.type How to calculate similarity between entries from both coding_indices and verbal answers from the survey? Three options are currently supported.
#' \describe{
...
#' }
#' @param tmp_folder The name of a folder where the algorithm will store the results from similarity calculations. Links between verbal answers and coding indices are only stored if the distance is \code{<= threshold[1]}
#'
#' @seealso
